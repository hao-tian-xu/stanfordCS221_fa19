...
def formula1c():
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return And(Or(Atom('Day'), Atom('Night')),
               Not(And(Atom('Day'), Atom('Night'))))
    # END_YOUR_CODE
...
def formula2a():
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return Forall('$x', Exists('$y', Implies(Atom('Person', '$x'), Atom('Mother', '$x', '$y'))))
    # END_YOUR_CODE
...
def formula2b():
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return Exists('$x', And(Atom('Person', '$x'), Forall('$y', Not(Atom('Child', '$x', '$y')))))
    # return Forall('$y', Exists('$x', Implies(Atom('Person', '$x'), Not(Atom('Child', '$x', '$y')))))
    # return Exists('$x', Forall('$y', Implies(Atom('Person', '$x'), Not(Atom('Child', '$x', '$y')))))
    # END_YOUR_CODE
...
def formula2c():
    ...
    return Forall('$x', Forall('$y', Equiv(And(Atom('Female', '$x'), Atom('Child', '$y', '$x')),
                                           Atom('Daughter', '$y', '$x'))))
    # END_YOUR_CODE
...
def formula2d():
    ...
    return Forall('$x', Forall('$z', Equiv(And(Atom('Female', '$x'),
                                               Exists('$y', And(Atom('Parent', '$z', '$y'),
                                                                Atom('Parent', '$y', '$x')))),
                                           Atom('Grandmother', '$z', '$x'))))
    # END_YOUR_CODE
...
def liar():
    ...
    # fact 1-3
    formulas += [Equiv(TellTruth(susan), CrashedServer(nicole)),
                 Equiv(TellTruth(mark), CrashedServer(susan)),
                 Equiv(TellTruth(nicole), Not(TellTruth(susan)))]
    # fact 4
    formulas.append(Exists('$x', And(TellTruth('$x'),
                                     Forall('$y', Implies(TellTruth('$y'), Equals('$x', '$y'))))))
    # fact 5
    formulas.append(Exists('$x', And(CrashedServer('$x'),
                                     Forall('$y', Implies(CrashedServer('$y'), Equals('$x', '$y'))))))
    # END_YOUR_CODE
...
def ints():
    ...
    formulas = [Forall('$x', Exists('$y', AndList([Successor('$x', '$y'),
                                                   Not(Equals('$x', '$y')),
                                                   Forall('$z', Implies(Successor('$x', '$z'), Equals('$z', '$y')))]))),
                Forall('$x', Or(And(Not(Even('$x')), Odd('$x')), And(Even('$x'), Not(Odd('$x'))))),
                Forall('$x', Forall('$y', Implies(And(Successor('$x', '$y'), Even('$x')), Odd('$y')))),
                Forall('$x', Forall('$y', Implies(And(Successor('$x', '$y'), Odd('$x')), Even('$y')))),
                Forall('$x', Forall('$y', Implies(Successor('$x', '$y'), Larger('$y', '$x')))),
                Forall('$x', Forall('$y', Implies(Larger('$x', '$y'),
                                                  Forall('$z', Implies(Larger('$y', '$z'), Larger('$x', '$z'))))))]
    # END_YOUR_CODE
