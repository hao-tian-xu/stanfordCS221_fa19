...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                beliefDist = math.sqrt((util.colToX(col) - agentX) ** 2 + (util.rowToY(row) - agentY) ** 2)
                emission_prob = util.pdf(beliefDist, Const.SONAR_STD, observedDist)
    ...
    def elapseTime(self):
        ...
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)
        # other's solution (utilizes class Belief and it's method addProb, and structure of transProb)
        ...
        # # my slow solution (slow: O(nTile^2)
        # num_rows = self.belief.getNumRows()
        # num_cols = self.belief.getNumCols()
        # self.belief.grid = [[sum(self.transProb.get(((_row, _col), (row, col)), 0) * self.belief.getProb(_row, _col)
        #                          for _col in range(num_cols) for _row in range(num_rows))
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        # re-weight according to observation
        new_weight = {}
        for tile in self.particles:
            beliefDist = math.sqrt((util.rowToY(tile[0]) - agentY) ** 2 + (util.colToX(tile[1]) - agentX) ** 2)
            emission_prob = util.pdf(beliefDist, Const.SONAR_STD, observedDist)
            new_weight[tile] = emission_prob * self.belief.getProb(*tile)
        # resample
        self.particles = collections.defaultdict(int)
    ...
    def elapseTime(self):
        ...
        for tile in self.particles:
            # re-weight according to transition probability
            new_weight = {newTile: self.transProb[(tile, newTile)]
                          for oldTile, newTile in self.transProb if oldTile == tile}
            # resample according to new weight
            for _ in range(self.particles[tile]):
                new_particles[util.weightedRandomChoice(new_weight)] += 1
